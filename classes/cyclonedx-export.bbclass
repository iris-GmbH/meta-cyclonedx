...
# Include unpatched vulnerabilities in the VEX
...
inherit ${@ "cve-check" if d.getVar("CYCLONEDX_INCLUDE_UNPATCHED_VULNS") == "1" else ""}
    ...
    for pkg in generate_packages_list(name, version):
        ...
        if d.getVar("CYCLONEDX_INCLUDE_UNPATCHED_VULNS") == "1" and os.path.exists(d.getVar("CVE_CHECK_DB_FILE")):
            with bb.utils.fileslocked([d.getVar("CVE_CHECK_DB_FILE_LOCK")], shared=True):
                bb.debug(2, f"Querying CVE database for unpatched CVEs for recipe {pn}")
                _, _, unpatched, _ = check_cves(d, patched_cves)
                bb.debug(2, f"Found {len(unpatched)} unpatched CVEs for recipe {pn}")
                for cve_id in unpatched:
                    cve = (
                        cve_id,
                        "Unpatched",
                        "no-fix-supplied",
