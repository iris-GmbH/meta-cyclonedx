...
def list_image_install_recipes(d):
    ...
    """
    return set(list_image_install_recipe_tokens(d).keys())
...
def list_image_install_recipe_tokens(d):
    """
    Return a mapping of recipe name -> IMAGE_INSTALL tokens that selected it.
    """
    image_install = d.expand(d.getVar("IMAGE_INSTALL") or "").split()
    recipe_tokens = {}

    def add_token(recipe, token):
        tokens = recipe_tokens.setdefault(recipe, set())
        tokens.add(token)

    def resolve_and_record(pkg_token, source_token):
        recipe = _resolve_runtime_token_to_recipe(d, pkg_token)
        if recipe:
            add_token(recipe, source_token)
        return recipe
    ...
    for token in image_install:
        ...
        root_recipe = resolve_and_record(token, token)
        if not root_recipe:
        ...
        if root_recipe.startswith("packagegroup-"):
            ...
            while queue:
                ...
                for dep_pkg in _read_runtime_package_rdepends(d, current_pkg_token):
                    dep_recipe = resolve_and_record(dep_pkg, token)
